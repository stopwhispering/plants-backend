...
from plants_tagger.models.taxon_occurence_image_models import TaxonOccurrenceImage
...
def _get_image_metadata(occ: dict, m: dict) -> Optional[dict]:
    ...
    try:
        d = {'occurrence_id':      occ['key'],
             ...
             'date':               m['created'] if 'created' in m else occ['eventDate'],
             ...
             # 'gbif_last_crawled_at': occ['lastCrawled'],
             # 'dataset_name':         occ.get('datasetName') or occ.get('collectionCode'),
             # 'decimal_longitude':    occ.get('decimalLatitude'),
             # 'decimal_latitude':     occ.get('decimalLongitude'),
             # 'gadm_level_0':         occ['gadm'].get('level0'),
             # 'gadm_level_1':         occ['gadm'].get('level1'),
             # 'gadm_level_2':         occ['gadm'].get('level2'),
             # 'gadm_level_3':         occ['gadm'].get('level3'),
             # 'country_code':         occ['countryCode'],
             # 'state_province':       occ.get('stateProvince'),
             # 'media_type':           m['type'],
             # 'title':                m.get('title'),  # most empty or irrelevant
             # 'format':               m['format'],
             # 'identified_by':        occ.get('identifiedBy'),
             # 'creator':              m.get('creator') or occ.get('recordedBy'),
             # 'publisher':            occ.get('publisher') or m.get('publisher') or occ.get('institutionCode')
             #                         or occ.get('rightsHolder'),
             # 'license':              m.get('license') or occ.get('license') or None,
             }
...
def _download_and_generate_thumbnail(info: Dict) -> Optional[str]:
    ...
    if os.path.isfile(os.path.join(PATH_GENERATED_THUMBNAILS_TAXON, filename)):
        logger.info(f'File already downloaded. Skipping download - {href}')
        return filename
...
def _treat_occurences(occs: List) -> List[Dict]:
    image_dicts = []
    for occ in occs:
        ...
        for j, m in enumerate(media, 1):
            ...
            d = _get_image_metadata(occ, m)
            if d:
                d['img_no'] = j

                if filename_thumbnail := _download_and_generate_thumbnail(d):
                ...
                # validate & convert
                try:
                    d = PTaxonOccurrenceImage(**d).dict()
...
def scrape_occurrences_for_taxon(gbif_id: int) -> List:
    ...
    image_dicts = _treat_occurences(occurrences)

    # save information to database
    new_list = []
    for img in image_dicts:
        record_new = TaxonOccurrenceImage(**img)
        new_list.append(record_new)
