...
def build_taxon_tree() -> List:

    # build up an exists filter that we're gonna reuse
    exists_filter = Taxon.plants.any(and_(or_(Plant.hide.is_(None), Plant.hide == False), Plant.active == True))

    # get distinct families, genus, and species (as list of four-element-tuples); sort
    dist_tuples = get_sql_session().query(Taxon.family, Taxon.genus, Taxon.species, Taxon.id).filter(
    ...
    for t in dist_tuples:
        ...
        plant_ids_tuple = get_sql_session().query(Plant.id).filter(Plant.taxon_id == t[3], or_(Plant.hide.is_(None),
                                                                                               Plant.hide == False),
                                                                   Plant.active == True).all()
