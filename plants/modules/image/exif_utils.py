...
def decode_record_date_time(date_time_bin: bytes) -> datetime.datetime:
    """Decode exif tag datetime to regular datetime object from b"YYYY:MM:DD HH:MM:SS" to datetime
    object."""
    try:
        ...
        s_format = "%Y-%m-%d"
    return datetime.datetime.strptime(s_dt, s_format).astimezone(pytz.timezone("Europe/Berlin"))


def encode_record_date_time(dt: datetime.datetime) -> bytes:
    """Encode datetime into format required by exif tag from datetime object to b"YYYY:MM:DD
    HH:MM:SS"."""
    s_format = "%Y:%m:%d %H:%M:%S"
...
def _auto_rotate_by_exif_flag(img: PilImage.Image, orientation_flag: int) -> PilImage.Image:
    if orientation_flag == 2:
        img = img.transpose(PilImage.FLIP_LEFT_RIGHT)
        logger.info(f"Rotating with orientation exif tag {orientation_flag}: flip left or right.")
        ...
        logger.info(f"Rotating with orient. exif tag {orientation_flag}: 180 & flip left to right.")
    elif orientation_flag == 5:
        img = img.rotate(-90, expand=True).transpose(PilImage.FLIP_LEFT_RIGHT)
        logger.info(f"Rotating with orient. exif tag {orientation_flag}: -90 & flip left to right.")
...
def auto_rotate_jpeg(path_image: Path, exif_dict: dict[str, Any]) -> None:
    """Auto-rotates images according to exif tag; required as chrome does not display them correctly
    otherwise; applies a recompression with high quality; re-attaches the original exif files to the
    new file but without the orientation tag."""
    if (
        not exif_dict
...
def exif_dict_has_all_relevant_tags(exif_dict: dict[str, Any]) -> bool:
    """The application uses most of all three exif tags to store information; returns whether all of
    them are extant in supplied exif dict."""
    try:
