...
async def get_photo(filename: str, width: int = None, height: int = None, image_dal: ImageDAL = Depends(get_image_dal)):
    image_bytes: bytes = await read_image_by_size(filename=filename, image_dal=image_dal, width=width, height=height)
...
async def get_occurrence_thumbnail(gbif_id: int,
    ...
    image_bytes: bytes = await read_occurrence_thumbnail(gbif_id=gbif_id,
