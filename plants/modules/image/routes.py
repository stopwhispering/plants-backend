...
async def upload_images_plant(request: Request,
    ...
    async with asyncio.TaskGroup() as tg:
        tasks = [tg.create_task(
            save_image_file(file=file, plant_names=plant_names)
        ) for file in files]
    paths: list[Path] = [task.result() for task in tasks]

    images: list[ImageCreateUpdate] = []
    for path in paths:
        images.append(await save_image_to_db(path=path,
                                             image_dal=image_dal,
                                             plant_dal=plant_dal,
                                             plant_ids=(plant.id,)))
...
async def upload_images(request: Request,
    ...
    # to be run concurrently - at least writing stops with "Session is already flushing"
    # InvalidRequestError) (2023-02)
    plant_names = [(await plant_dal.by_id(plant_id)).plant_name for plant_id in additional_data['plants']]
    async with asyncio.TaskGroup() as tg:
        tasks = [tg.create_task(
            save_image_file(file=file, plant_names=plant_names, keywords=additional_data['keywords'])
        ) for file in files]
    paths: list[Path] = [task.result() for task in tasks]
