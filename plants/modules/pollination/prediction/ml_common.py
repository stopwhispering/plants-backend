...
def unpickle_pipeline(
    ...
    return_category_map: bool = False,  # noqa: FBT002
) -> (
    tuple[Pipeline | VotingRegressor | VotingClassifier | XGBClassifier, FeatureContainer]
    | tuple[Pipeline | VotingRegressor | VotingClassifier | XGBClassifier, FeatureContainer, dict]
):
    ...
    with path.open("rb") as file:
        try:
            ...
            throw_exception(
                f"Error unpickling pipeline from {path.as_posix()}: {exc}. Probably "
                f"version issues."
            )
