...
class ImageDAL(BaseDAL):
    ...
    async def get_distinct_image_keywords(self) -> set[str]:
        ...
        image_keywords: list[ImageKeyword] = (await self.session.scalars(query)).all()  # noqa
    ...
    async def delete_image_to_event_associations(self, image: Image, links: list[ImageToEventAssociation]):
        for link in links:
            image.image_to_event_associations.remove(link)
            await self.session.delete(link)
        await self.session.flush()

    async def delete_image_to_plant_associations(self, image: Image, links: list[ImageToPlantAssociation]):
        for link in links:
            image.image_to_plant_associations.remove(link)
            await self.session.delete(link)
        await self.session.flush()

    async def delete_image_to_taxon_associations(self, image: Image, links: list[ImageToTaxonAssociation]):
        for link in links:
            image.image_to_taxon_associations.remove(link)
            await self.session.delete(link)
        await self.session.flush()

    async def delete_keywords_from_image(self, image: Image, keywords: list[ImageKeyword]):
        for keyword in keywords:
            image.keywords.remove(keyword)
            await self.session.delete(keyword)
        await self.session.flush()

    async def create_new_keywords_for_image(self, image: Image, keywords: list[ImageKeyword]):
